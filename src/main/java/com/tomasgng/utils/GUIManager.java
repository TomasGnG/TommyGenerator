...
public class GUIManager {
    ...
    public void openWorldEditInventory(Player player, String displayName, World world) {
        ...
        var setMonsterSpawningItem = new ItemBuilder(Material.CREEPER_HEAD).setKey("worldEditInv-setMonsterSpawningItem").setDisplayName("§aToggle monster spawning").setLore("§8§o→ Toggles the spawning of monsters like zombies etc.", "§8§o→ §c§oIt takes a couple of seconds till the state gets updated..", "§8→ §f" + TommyGenerator.getInstance().getWorldManager().monsterSpawning(world)).build();
